...
public sealed class MadladTranslationEngine : ITranslationEngine, IStageRuntimeExecutionReporter
{
    private readonly IRuntimePlanner runtimePlanner;
    private readonly BenchmarkModelPathResolver modelPathResolver;

    public MadladTranslationEngine(
        ...
        CancellationToken cancellationToken)
    {
        ...
        MadladTokenizerDecoder tokenizer = MadladTokenizerDecoder.Load(modelRootPath);
    ...
    {
        string modelRootPath = ResolveModelRootPath(encoderModelPath);
        foreach (string fileName in new[] { "decoder_model_int8.onnx", "decoder_model.onnx", "decoder_model_merged.onnx" })
        {
